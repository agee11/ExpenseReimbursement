	...
	xhttp.onreadystatechange = function(){
		...
		if(xhttp.readyState == 4 && xhttp.status == 200){
			...
			item.forEach(element => {
				...
				document.getElementById("table-caption").innerHTML = "Employee ID: " + element.employeeId;
...
function logout(){
	let xhttp = new XMLHttpRequest();
	let url = "http://localhost:8080/ExpenseReimbursement/api/logout";
	xhttp.open("GET", url);
	xhttp.send();
	
	xhttp.onreadystatechange = function() {
		if(xhttp.readyState == 4 && xhttp.status == 200){
			window.location.replace("http://localhost:8080/ExpenseReimbursement/api");
		}
	}
}
